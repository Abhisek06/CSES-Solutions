...
void solve(){
	...
	while(!pq.empty()){
		...
		if(dis[node.se][k-1]<node.fi)
		continue;
		for(auto child:adj[node.se]){
			if(dis[child.fi][k-1]>node.fi+child.se){
				dis[child.fi][k-1]=node.fi+child.se;
				pq.push({dis[child.fi][k-1],child.fi});
				sort(all(dis[child.fi]));
			}
		}
	}
	for(ll i=0;i<k;i++){
